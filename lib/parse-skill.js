...
/**
 * Filename match alone isn't enough — plenty of unrelated repos (games, docs,
 * tutorials) happen to have a file literally named SKILL.md. This checks the
 * content actually looks like an agent-skill file before it's accepted into
 * the crawl pipeline at all.
 *
 * Heuristic, not perfect: requires frontmatter with a name/description field
 * OR strong contextual signals (mentions of "Claude", "agent", "tool use",
 * instructions written in second-person imperative to an AI). Tune the
 * signal list over time as false positives/negatives show up in review.
 */
export function looksLikeAgentSkill(raw) {
  const fmMatch = raw.match(/^---\s*\n([\s\S]*?)\n---/);
  if (fmMatch) {
    const fm = fmMatch[1];
    const hasName = /^name:\s*.+$/m.test(fm);
    const hasDescription = /^description:\s*.+$/m.test(fm);
    if (hasName && hasDescription) return true;
  }
  ...
  const contextSignals = [
    ...
    /\btool[\s-]?use\b/i,
    /\bSKILL\.md\b/,
    /when (to use|the user asks|this skill)/i,
  ];
  const matchCount = contextSignals.filter((re) => re.test(raw)).length;
